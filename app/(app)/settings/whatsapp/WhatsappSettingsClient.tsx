...
import { Check, ChevronLeft, Eye, EyeOff, Info, Loader2, Smartphone, Zap } from "lucide-react";
  ...
  return (
    <div className="animate-fade-up max-w-2xl mx-auto space-y-8">
      <div className="space-y-1">
        <Link
          href="/settings"
          className="inline-flex items-center text-[10px] font-bold text-text-muted hover:text-accent uppercase tracking-widest transition-colors mb-2 gap-1"
        >
      ...
      <div className="grid gap-4">
        <button
          ...
          className={`w-full text-left p-5 rounded-2xl border-2 transition-all relative overflow-hidden group ${
            mode === "manual" ? "bg-accent-light border-accent shadow-sm" : "bg-white border-border hover:border-border-strong"
          }`}
        >
          <div className="flex justify-between items-start">
            <div className="space-y-1 pr-8">
              <h3 className={`text-sm font-bold uppercase tracking-wide ${mode === "manual" ? "text-accent-text" : "text-text-primary"}`}>
                Manual reminders
              </h3>
              <p className="text-xs text-text-secondary font-medium leading-relaxed max-w-[400px]">
                One-tap sending. RepCore opens WhatsApp with a personalized message and you just hit send. Ideal for relationship building.
              </p>
            </div>
            <span className="bg-accent text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest whitespace-nowrap">
              Free
            </span>
          </div>
          {mode === "manual" && (
            <div className="absolute right-4 bottom-4 h-6 w-6 bg-accent rounded-full flex items-center justify-center text-white">
        ...
        <button
          ...
          className={`w-full text-left p-5 rounded-2xl border-2 transition-all relative overflow-hidden group ${
            mode === "auto" ? "bg-accent-light border-accent shadow-sm" : "bg-white border-border hover:border-border-strong"
          }`}
        >
          <div className="flex justify-between items-start">
            <div className="space-y-1 pr-8">
              <h3 className={`text-sm font-bold uppercase tracking-wide ${mode === "auto" ? "text-accent-text" : "text-text-primary"}`}>
                Auto reminders
              </h3>
              <p className="text-xs text-text-secondary font-medium leading-relaxed max-w-[400px]">
      ...
      <div className="bg-status-info-bg border border-status-info-border rounded-2xl p-5 flex gap-4 animate-fade-in">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-status-info-text leading-relaxed font-medium">
            Connect to any WhatsApp Business API (like AiSensy or Gupshup). You maintain full control of your messaging wallet and template approvals on your provider dashboard.
          </p>
        </div>
      </div>
          ...
          <form onSubmit={handleSaveConfig} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-text-muted font-semibold uppercase tracking-wider">
                ...
                <a href="https://aisensy.com" target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">
                  Get an API key
                </a>
              </p>
            </div>

            <Button type="submit" disabled={isPending} className="w-full h-12 uppercase tracking-[0.2em] text-xs">
