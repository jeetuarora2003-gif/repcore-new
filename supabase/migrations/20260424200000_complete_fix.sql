/*
  ...
  This script safely drops all existing functions and views, 
  then recreates them from scratch to avoid any conflicts.
*/

-- ============================================================
-- STEP 1: Drop ALL functions (with any signature) and the view
-- ============================================================
...
DROP FUNCTION IF EXISTS clean_old_attendance();
...
-- ============================================================
-- STEP 5: Create subscription renewal RPC
-- ============================================================
...
-- ============================================================
-- STEP 6: Create payment recording RPC
-- ============================================================
...
-- ============================================================
-- STEP 7: Create dashboard stats RPC (RETURNS jsonb, not json)
-- ============================================================
CREATE FUNCTION get_dashboard_stats(p_gym_id uuid)
RETURNS jsonb AS $$
DECLARE
...
-- ============================================================
-- STEP 8: Attendance cleanup function
-- ============================================================
...
-- ============================================================
-- STEP 9: Data integrity (safe — skips if already exists)
-- ============================================================
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'members_gym_phone_unique'
  ) THEN
    DELETE FROM members a USING members b
    WHERE a.gym_id = b.gym_id AND a.phone = b.phone AND a.phone <> '' AND a.created_at < b.created_at;

    ALTER TABLE members ADD CONSTRAINT members_gym_phone_unique UNIQUE (gym_id, phone);
  END IF;
END $$;

-- ============================================================
-- STEP 10: Attendance indexes
-- ============================================================
